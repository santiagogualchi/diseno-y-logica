...
hábitos para su almacenamiento. Por último, vamos a introducir una forma de
...
El archivo README
  ...
  archivo README resume la información esencial que el usuario debe conocer al
  hacer uso de esa solución, y es el primer archivo que el usuario debe
  consultar cuando accede a un proyecto. Los archivos README suelen estar
  escritos en texto plano (sin ningún tipo de formato) o usando algún lenguaje
  de marcado (por lo general, MarkDown). Es fundamental que todo proyecto que
  encaremos cuente con un buen archivo README! Pueden ver un ejemplo de
...
El directorio src
  ~ En esta carpeta va a estar contenido todo el código (excepto el que usemos
  en las notebooks) que escribamos para   llevar a cabo nuestro análisis, como
  el que se ocupa de descargar, generar o limpiar los datos, definir y entrenar
  los modelos, o crear visualizaciones.
 ...
 * enunciados con una estructura condicional (*si..., entonces...*), o que, al
   menos, puede ser parafraseados como tales; y
...
o relaciones entre variables ocurran en una dirección determinada (se desvían
...
recogida en el primer diagrama de barras de la [Figura 1](#dists). Como se
observa en dicha figura, a medida que aumenta el número de sujetos la
distribución se asemeja cada vez más a la de la distribución gaussiana o normal.
...
```{R, include = FALSE}
subj3 = tibble(n = 0:3, `\np` = dbinom(0:3, 3, 0.5))
subj6 = tibble(n = 0:6, p = dbinom(0:6, 6, 0.5))
subj11 = tibble(n = 0:11, p = dbinom(0:11, 11, 0.5))
subj25 = tibble(n = 0:25, `\np` = dbinom(0:25, 25, 0.5))
subj50 = tibble(n = 0:50, `\np` = dbinom(0:50, 50, 0.5))
subj100 = tibble(n = 0:100, `\np` = dbinom(0:100, 100, 0.5))
```
...
```{R, lab, echo = FALSE, fig.cap = "<span id=\"dists\">Distribución</span> de probabilidad para resultados de 3, 6, 12, 25, 50 y 100 intentos binarios igualmente probables."}
s3p <- ggplot(data = subj3, aes(n, `\np`)) +
  geom_bar(stat = "identity",  color = "black", fill = "white") +
  coord_cartesian(ylim=c(0,0.4)) +
  theme_minimal()

s6p <- ggplot(data = subj6, aes(n, p)) +
  geom_bar(stat = "identity",  color = "black", fill = "white") +
  coord_cartesian(ylim=c(0,0.4)) +
  theme_minimal()

s11p <- ggplot(data = subj11, aes(n, p)) +
  geom_bar(stat = "identity",  color = "black", fill = "white") +
  coord_cartesian(ylim=c(0,0.4)) +
  theme_minimal()

s25p <- ggplot(data = subj25, aes(n, `\np`)) +
  geom_bar(stat = "identity",  color = "black", fill = "white") +
  coord_cartesian(ylim=c(0,0.4)) +
  theme_minimal()

s50p <- ggplot(data = subj50, aes(n, `\np`)) +
  geom_bar(stat = "identity",  color = "black", fill = "white") +
  coord_cartesian(ylim=c(0,0.4)) +
  theme_minimal()

s100p <- ggplot(data = subj100, aes(n, `\np`)) +
  geom_bar(stat = "identity",  color = "black", fill = "white") +
  coord_cartesian(xlim=c(0, 101), ylim=c(0,0.4)) +
  theme_minimal()

multiplot(s3p, s25p, s6p, s50p, s11p, s100p, cols = 3)
```
...
los sujetos respondan *nombre* 59 veces o más es de $p = 0.044$ (véase
[Figura 2](#negro)).

```{R, echo = FALSE, fig.height = 2, fig.cap = "<span id=\"negro\">En</span> negro, probabilidad de obtener 59 respuestas o más que clasifican camino como un nombre."}

ggplot(data = subj100, aes(n, `\np`)) +
  geom_bar(
    stat = "identity",
    color = "black",
    fill = c(rep("white", 59), rep("black", 42))
    ) +
  theme_minimal()
...
En la sección anterior, nuestra H~1~ era direccional: “Si una palabra puede ser
...
veces). Así, obtenemos una frecuencia acumulada de $p = 0.089$ (véase
[Figura 3](#negro2)).
Dado que este resultado es mayor al _p_~crítico~ que definimos, no podemos
rechazar H~0~.

```{R, echo = FALSE, fig.height = 2, fig.cap = "<span id=\"negro2\">En</span> En negro, probabilidad de obtener 41 respuestas o menos y 59 respuestas o más que clasifican camino como un nombre."}

ggplot(data = subj100, aes(n, `\np`)) +
  geom_bar(
    stat = "identity",
    color = "black",
    fill = c(rep("black", 42), rep("white", 17), rep("black", 42))
    ) +
  theme_minimal()
